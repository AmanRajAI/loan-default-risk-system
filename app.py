...
def startup_log(stage):
    ...
    print(
        f"[startup] {time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime())} | {stage}",
        flush=True
    )
...
def load_model():
    """
    Load the ML model once and keep it cached.

    This is intentionally separated from portfolio data loading
    so the model does not need to be reloaded when data changes.
    """

    try:
        startup_log("loading model artifacts")

        with st.spinner("Loading loan risk model..."):
        ...
        print(
            "[startup] model artifact loading failed",
            flush=True
        )
...
# ============================================================
# OPTIMIZED PORTFOLIO DATA LOADING
# ============================================================

@st.cache_data
def load_portfolio_data():
    """
    Download and load the small deployment portfolio dataset.

    Streamlit caches the returned DataFrames separately from
    the model, avoiding unnecessary repeated work.
    """
    ...
    try:
        ...
        print(
            "[startup] calling load_test_data",
            flush=True
        )

        X_sample, y_sample = load_test_data(
            test_path
        )

        print(
            "[startup] load_test_data completed",
            flush=True
        )

        # Limit portfolio data to 5,000 rows
        sample_size = min(
            5000,
            len(X_sample)
        )

        sample_indices = X_sample.sample(
            sample_size,
            random_state=42
        ).index
        ...
        print(
            "[startup] portfolio data loading failed",
            flush=True
        )
        ...
        st.warning(
            f"⚠️ Could not load test data: {str(e)}"
        )
...
def create_kpi_metrics(explainer, X_data):
    ...
    risk_dist = explainer.get_risk_distribution(
        X_data
    )
...
def show_individual_assessment(
    ...
    with col1:
        ...
        if st.checkbox(
            "Total Accounts Unknown",
            key="total_acc_unknown"
        ):
            total_accounts = None
        ...
        if st.checkbox(
            "Open Accounts Unknown",
            key="open_acc_unknown"
        ):
            open_accounts = None
    ...
    with col2:
        ...
        if st.button(
            ...
            try:
                ...
                model_input = (
                    convert_user_inputs_to_model_features_flexible(
                ...
                # Prediction
                pred_info = explainer.predict_loan(
                    model_input
                )

                decision_color = (
                    "🟢"
                    if pred_info["decision"] == "Approved"
                    else "🔴"
                )
                ...
                with col_risk2:
                    ...
                    factors_text += (
                        f"- FICO Score: {fico_score} "
                        f"{'✅' if fico_score >= 650 else '❌'}\n"
                    )

                    factors_text += (
                        f"- Credit Sub-Grade: {sub_grade_letter} "
                        f"{'✅' if sub_grade <= 3.0 else '❌'}\n"
                    )
                    ...
                    if credit_utilization is not None:
                        ...
                        factors_text += (
                            ...
                            f"{credit_utilization:.0f}% "
                            f"{'✅' if credit_utilization <= 30 else '❌'}\n"
                        )

                    if employment_length != "Unknown":

                        factors_text += (
                            f"- Employment: {employment_length} "
                            f"{'✅' if employment_length not in ['< 1 year', 'Unknown'] else '❌'}\n"
                        )
                ...
                with st.expander(
                    ...
                    try:
                        ...
                        st.warning(
                            "Could not generate SHAP explanation: "
                            f"{str(shap_error)}"
                        )
...
def create_sub_grade_input():
    """Create sub-grade input with A1-G5 options."""

    grade_options = []
    grade_values = []
    grade_labels = []

    for grade_idx, grade_letter in enumerate(
        ["A", "B", "C", "D", "E", "F", "G"],
        1
    ):
        ...
        for sub_idx in range(1, 6):
            ...
            grade_label = (
                f"{grade_letter}{sub_idx}"
            )

            grade_display = (
                f"{grade_label} ({grade_value})"
            )
...
def convert_sub_grade_to_letter(
    sub_grade_value
):
    """Convert numeric sub-grade to letter format."""
...
def convert_user_inputs_to_model_features_flexible(
    annual_income,
    employment_length,
    home_ownership,
    verification_status,
    ...
    # Fill every model feature with NaN
    ...
    input_df["annual_inc_log"] = (
        np.log1p(annual_income)
    )
    ...
    if delinquencies_2yrs is not None:
        ...
        input_df["delinq_2yrs"] = (
            delinquencies_2yrs
        )

    if credit_inquiries is not None:

        input_df["inq_last_6mths"] = (
            credit_inquiries
        )
    ...
    target_purpose = (
        f"purpose_{loan_purpose}"
    )

    if target_purpose in input_df.columns:

        input_df[target_purpose] = 1
    ...
    if (
        "revolutil_fico" in X_sample.columns
        and credit_utilization is not None
    ):
...
def show_batch_analysis(
    ...
    """Enhanced batch loan risk analysis."""

    st.header(
        "📊 Portfolio Batch Risk Analysis"
    )
    ...
    with col1:
        ...
        st.markdown(
            ...
            **Portfolio Risk Assessment:**
            Analyze risk distribution across your loan portfolio
            to identify concentration risks, optimal approval rates,
            and expected loss patterns.

            This analysis helps in portfolio optimization and
    ...
    with col2:
        ...
        sample_size = st.slider(
            ...
            step=1000,
            help=(
                "Larger samples provide more accurate risk estimates "
    ...
    if st.button(
        ...
        with st.spinner(
            ...
            show_risk_distribution_analysis(
                batch_results,
                X_batch,
                y_batch
            )
...
def show_risk_distribution_analysis(
    batch_results,
    X_batch,
    y_batch
):
    ...
    with col3:
        ...
        st.metric(
            ...
            help=(
                ...
                "applications (>30% default probability)"
            )
        )
...
def plot_threshold_analysis(
    ...
    threshold_df = (
        explainer.simulate_threshold_impact(
            X_data
        )
    )
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            ...
            "Expected Bad Rate vs Threshold"
        ],
    ...
    fig.add_trace(
        go.Scatter(
            x=threshold_df["threshold"],
            y=threshold_df["expected_bad_rate"],
            mode="lines+markers",
    ...
    fig.update_layout(
        height=500,
        title_text="Threshold Impact Analysis"
    )
...
def main():
    ...
    st.title(
        "🏦 Loan Default Risk System Dashboard"
    )

    st.markdown(
        "Advanced ML-powered loan risk assessment "
    ...
    if page == "📊 Executive Dashboard":
        ...
        if X_sample is None:
            ...
            st.info(
                ...
                "This feature requires portfolio data. "
                "Please try the '🎯 Risk Assessment' page "
                "for individual loan analysis."
            )

        else:

            risk_dist = create_kpi_metrics(
                explainer,
                X_sample
            )
            ...
            with col2:
                ...
                st.metric(
                    "Mean Risk",
                    f"{risk_dist['mean_risk']:.3f}"
                )

                st.metric(
                    "Risk Std Dev",
                    f"{risk_dist['std_risk']:.3f}"
                )
        ...
        if X_sample is None:
            ...
            with tab1:
                ...
                try:
                    ...
                    st.error(
                        f"Error creating SHAP plot: {str(e)}"
                    )
            ...
            with tab2:
                ...
                selected_feature = (
                    st.selectbox(
                        ...
                        options=explainer.feature_names[:20]
                    )
                )
                ...
                if st.button(
                    ...
                    try:
                        ...
                        dep_fig = (
                            explainer.create_dependence_plot(
                                X_sample.head(500),
                                selected_feature
                            )
                        )
                        ...
                        st.error(
                            f"Error creating dependence plot: {str(e)}"
                        )
    ...
    # ========================================================
        ...
        st.header(
            "Portfolio Optimization & Threshold Tuning"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "[startup] main application failed; "
            "traceback follows",
            flush=True
        )
